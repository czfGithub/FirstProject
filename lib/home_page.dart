...
import 'package:flutterapp/utils/net_util.dart';
...
class _PopupRoutePageState extends State<PopupRoutePage> {
  ...
  /// 登录
  Future<User> getSmsCode(BuildContext context, String phone) async {

    User user = await NetUtil.getSmsCode(context, phone);
    if (user.code != 0) {
      Utils.showToast(user.msg ?? '验证码获取失败');
      return null;
    }
    Utils.showToast(user.msg ?? '${user.msg}');
    reGetCountdown();
    return user;
  }
...
//                        keyboardType: TextInputType.number,
                    ...
                    FlatButton(
                      ...
                      onPressed: (){
                        String phone = editingController.text;
                        if(phone == ''){
                          Utils.showToast('请输入手机号');
                          return;
                        }
                        if(phone.length != 11){
                          Utils.showToast('手机号不正确');
                          return;
                        }
                        getSmsCode(context, phone).then((value){
                          if(value != null){
